...
#define	AGENT_TOKEN_ID		"token_id"
...
agent_request_search(vdev_queue_t *vq, uint64_t blockid)
{
	 ...
	 * a give blockid (see command in agent_request_zio for more
	 * info).
	 */
...
/*
 * Send request to agent; returns request ID (search key in
 * vdev's vq_active_tree).  nvlist may be modified.
 */
static void
agent_request_zio(vdev_object_store_t *vos, zio_t *zio, nvlist_t *nv)
{
	...
	fnvlist_add_uint64(nv, AGENT_TOKEN_ID, (uint64_t)zio);
...
agent_reader(void *arg)
{
	...
	if (strcmp(type, "pool create done") == 0) {
		...
		uint64_t req = fnvlist_lookup_uint64(nv,
		    AGENT_REQUEST_ID);
		uintptr_t token = fnvlist_lookup_uint64(nv, AGENT_TOKEN_ID);
		uint_t len;
