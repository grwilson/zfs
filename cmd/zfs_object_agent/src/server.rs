...
impl Server {
            ...
            loop {
                ...
                match nvl.lookup_string("Type").unwrap().to_str().unwrap() {
                    ...
                    "write block" => {
                        ...
                        let token_id = nvl.lookup_uint64("token_id").unwrap();
                        if let NvData::Uint8Array(slice) = data {
                            ...
                            server.write_block(block, slice.to_vec(), id, token_id);
                        } else {
                    ...
                    "read block" => {
                        ...
                        let id = nvl.lookup_uint64("request_id").unwrap();
                        let token_id = nvl.lookup_uint64("token_id").unwrap();
                        server.read_block(block, id, token_id);
                    }
                    other => {
                        panic!("bad type {:?} in request {:?}", other, nvl);
                    }
    ...
    fn write_block(&mut self, block: BlockId, data: Vec<u8>, request_id: u64, token_id: u64) {
        ...
        tokio::spawn(async move {
            ...
            nvl.insert("token_id", &token_id).unwrap();
    ...
    fn read_block(&mut self, block: BlockId, request_id: u64, token_id: u64) {
        let pool = self.pool.as_ref().unwrap().clone();
        let output = self.output.clone();
        tokio::spawn(async move {
